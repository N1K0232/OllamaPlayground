...
namespace OllamaPlayground.ApiService.Swagger;
...
public class SwaggerBasicAuthenticationMiddleware(RequestDelegate next, IOptions<SwaggerSettings> swaggerOptions)
{
    ...
    {
        if (httpContext.Request.Path.StartsWithSegments("/swagger") && swagger.UserName.HasValue() && swagger.Password.HasValue())
        {
